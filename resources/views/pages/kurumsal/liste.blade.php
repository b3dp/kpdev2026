...
@php
    $metaDescription = 'Kurumsal sayfalar: Kestanepazari tarihce, tüzük, faaliyet raporlari ve diger kurumsal basliklar.';
    $varsayilanOg = 'https://cdn.kestanepazari.org.tr/logo.png';
    $ilkGorsel = $standartSayfalar->first()?->kart_gorseli;
    $ogImage = filled($ilkGorsel) ? $ilkGorsel : $varsayilanOg;
@endphp

@section('title', 'Kurumsal')
@section('meta_description', $metaDescription)
@section('canonical', route('kurumsal.show'))
@section('og_image', $ogImage)

@section('content')
    <section class="mx-auto max-w-7xl px-4 py-10 lg:px-6 lg:py-12">
        <div class="mb-8">
            <h1 class="font-baskerville text-3xl font-bold text-primary md:text-5xl">Kurumsal</h1>
        </div>

        @if($standartSayfalar->isEmpty())
            <div class="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-500">
                Kurumsal sayfa bulunamadi.
            </div>
        @else
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                @foreach($standartSayfalar as $sayfa)
                    @php
